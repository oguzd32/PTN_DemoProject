...
namespace XDPaint.Core.Materials
{
	...
	public class Brush
	{
		...
		private void InitMaterials()
		{
			...
			//next line can be used if PaintManager does not have source texture (RenderComponentsHelper will create transparent texture for painting)
		...
		public void SetPaintTool(PaintTool paintTool)
		{
			...
			if (paintTool == PaintTool.Erase)
			{
				...
				material.SetInt(SrcAlphaBlend, (int) BlendMode.Zero);
			}
			else
			{
				material.SetInt(SrcColorBlend, (int) BlendMode.SrcAlpha);
				material.SetInt(SrcAlphaBlend, (int) BlendMode.One);
				//next line can be used if PaintManager does not have source texture (RenderComponentsHelper will create transparent texture for painting)
