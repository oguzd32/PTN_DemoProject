	...
	public class PaintController : Singleton<PaintController>
	{
		...
		public bool Preview
		{
			...
			set
			{
				...
				{
					preview = value;
					brush.SetPaintTool(_toolsManager.CurrentTool.Type);
					foreach (var paintManager in _allPaintManagers)
					{
		...
		{
			var paintManagers = _allPaintManagers.Where(paintManager => paintManager != null && paintManager.gameObject.activeInHierarchy && paintManager.enabled && paintManager.Initialized).ToList();
			return paintManagers.ToArray();
		}
	}
}